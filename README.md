...
- call
- staticcall
- delegatecall，代理合约的时候一般会用到，还有一个是钻石？提供了一定灵活性，但要确保当前合约跟目标合约状态变量存储结构一致(面临挑战)否则会出问题。允许合约A调用合约B的代码却在A的上下文中执行
为解决 delegatecall 的问题，社区发展了如下两种主流升级模式
- Transparent Proxy，透明代理模式
- Universal Upgradeable Proxy Standard，UUPS升级代理模式
但是无论哪种升级模式，都得解决存储冲突问题，因此 ```EIP-1967```提出了标准化的解决方案
